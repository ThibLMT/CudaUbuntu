...
unsigned int * allocation_backgrid(geom_struct geom)
  ...
  return malloc(nb_elements * sizeof(unsigned int));
  }
int * allocation_backgrid_insert(geom_struct geom)
  {
  int nb_elements;
  nb_elements=geom.sizex*geom.sizey*geom.sizez;
  return malloc(nb_elements * sizeof(int));
  }
  
void initialize_backgrid(unsigned int *backgrid,int *backgrid_insert,geom_struct geom)
  {
  int indvox,size_backgrid,size_backgrid_insert;
  size_backgrid_insert=geom.sizex*geom.sizey*geom.sizez;
  size_backgrid=size_backgrid_insert*geom.sizel;

#pragma omp parallel shared(backgrid_insert,backgrid,size_backgrid_insert,size_backgrid) private(indvox)
  {
  #pragma omp for    
    for(indvox=0;indvox<size_backgrid_insert;indvox++) 
      {backgrid_insert[indvox]=0;}
    
    
  #pragma omp for
    for(indvox=0; indvox<size_backgrid;indvox++)
      {backgrid[indvox]=0;}
  }
 }
